...
st.subheader("📝 Dados da Decisão (mesma para todas as recomendeção)")
...
# --- 🔍 CAMPO CORRIGIDO: Status da Ação apresentada peloGestor ---
status_acao = st.selectbox(
    "🔧Status da Ação apresentada peloGestor",
...
if st.session_state.analise_feita:
    ...
    # --- 📝 ANÁLISEDa AÇÃO DO GESTOR (com prompt isolado) ---
    ...
    try:
        ...
        5. Retorne apenas um texto claro, técnico e objetivo, com até 150palavras.
        ...
        elif "⚠️Parcialemente" in analise_acao:
            st.session_state.data["classificacao final"] = "⚠️Parcialemente compatível"
...
if pergunta:
    with st.spinner("Buscando no histórico..."):
        try:
            contexto = ""
            try:
                df = pd.read_csv("memória/historico.csv")
                candidatos = df[
