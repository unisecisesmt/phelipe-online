...
1. 📚 ANÁLISE MULTIDOCUMENTAL (Contexto Técnico)
   ...
   -Decisão do TCE-MT: Recomendação específicaa, prazo, responsabilidades
   -Resoste do Gestor: Ação informada, prazo, evidência anexada

2. ⏳ RECONSTRUÇÃO da CRonoLOGIE
   Ordene os event:
   - O que foi constatado?
   - Como o MPC opinou?
   - O que decideiram os conselheiros?
...
# ---FUNÇÃO PARA EXTRARRETTO DE PDFs (SEMOCR) ---
...
st.subheader("📝 Dados da Decisão (mes ma para todas as recomendações)")
num_recisao = st.text_input("🔢 Número da Decisão (ex: Acórdão 1234/2025)")
data_recisao = st.text_input("📅 Data da Decisão")
num_processo_tce = st.text_input("📋 Número do Processos no TCE")
        ...
        try:
            ...
            try:
                ...
                df = pd.DataFrame([{
                    ...
                    "observacoes_memoria": data.get("observacoes_memoria", "Nenhuma observação registrada.")
                }])
...
if st.session_state.analise_feita:
    ...
    try:
        ...
        Você é Phelipe, um especialista técnico em controle interno, controle externo, SES/MT, integridade e normy aplicáveis.
        ...
        4. Classifique com base nisso:
           ...
           - ⚠️Parcelemente compatível: ação incompleta, sem evidência ou com risco alto
           - ❌ Incompatível: ação irrelevante, contradadiz a recomendação ou não corrige o problema
        ...
        elif "⚠️Parcelemente" in analise_acao:
            st.session_state.classificacao_final = "⚠️Parcelemente compatível"
    ...
    st.write("**Padrões identificadosos:**")
    for p in insight.get("padroes_identificadosos", []):
        st.write(f"• {p}")
    ...
    if dano.get("consta_rano"):
        ...
        st.caption(f"Font: {dano.get('fundamentacao', 'não consta')}")
    else:
            ...
            try:
                ...
                if os.path.exists("memoria/historico.csv"):
                    ...
                    if not candidatos.empty:
                        contexto += "📌 Caseos semelhantes encontrados:\n"
                        for _, row in candidatos.iterrows():
                            contexto += f"- {row['num_recisao']}: {row['recomendacao'][:100]}...\n"
                else:
                    contexto += "⚠️Histórico vazio ou não encontrado.\n"
            except Exception as e:
